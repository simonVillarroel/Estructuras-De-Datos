...
public class ArbolBB {
    ...
    public ArbolBB clonarParteInvertida(Comparable elem) {
        ...
        clonInvertido.raiz = parteInvertida(this.raiz, elem);
        return clonInvertido;
    }

    private NodoABB parteInvertida(NodoABB nodo, Comparable elem) {
        NodoABB nodoClon = null;
        if (nodo != null) {
            if (elem.compareTo(nodo.getElem()) == 0) {
                nodoClon = invertir(nodo);
            }else if (elem.compareTo(nodo.getElem()) < 0) {
                    nodoClon = parteInvertida(nodo.getIzquierdo(), elem);
                } else {
                    nodoClon = parteInvertida(nodo.getDerecho(), elem);
                }
        }
        return nodoClon;
    }
    ...
    private NodoABB invertir(NodoABB nodo){
        ...
        if(nodo != null){
            nodoClon = new NodoABB(nodo.getElem());
            nodoClon.setIzquierdo(invertir(nodo.getDerecho()));
            nodoClon.setDerecho(invertir(nodo.getIzquierdo()));
        }
        return nodoClon;
    }
}
